...
func Sparse(runenv *runtime.RunEnv, initCtx *run.InitContext) error {
	...
	// Get sequence number of this host
	_, err = client.Publish(ctx, peerInfos, &peerInfo{*nConfig.AddrInfo, nodetp})
	if err != nil {
		return err
	}
	...
	infoCh := make(chan *peerInfo)
	sctx, cancelSub := context.WithCancel(ctx)
	if _, err = client.Subscribe(sctx, peerInfos, infoCh); err != nil {
		cancelSub()
		return err
	}
	// var infos []peerInfo
	...
	// }
	infos, err := PeerInfosFromChan(infoCh, runenv.TestInstanceCount)
	if err != nil {
		cancelSub()
	...
	for fIndex, f := range testFiles {
		...
		for runNum = 1; runNum < testvars.RunCount+1; runNum++ {
			...
			dialed, err := SparseDial(ctx, ipfsNode.Node.PeerHost, nodetp, infos, testvars.MaxConnectionRate)
			if err != nil {
				return err
			}
			runenv.RecordMessage("%s Dialed %d other nodes:", nodetp.String(), len(dialed))
			...
			if nodetp == utils.Leech || nodetp == utils.Passive {
				// Clearing datastore
